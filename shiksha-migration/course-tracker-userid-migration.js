...
console.log('=== Loading course-tracker-userid-migration.js ===');
...
//   OPERATION 1 : Fetch all user_course_certificate rows for destUserId
//   OPERATION 2 : For each entry's courseId:
//     a) sourceUserId has NO record for that courseId
//          → UPDATE: set userId = sourceUserId on that row
//     b) sourceUserId ALREADY has a record for that courseId
//          → Check status priority:
//              destUser priority > sourceUser priority
//                → UPDATE sourceUser's status to destUser's status
//              else
//                → SKIP (do nothing)
const DEST_USER_IDS = [
...
async function migrateCourseTrackerUserIds() {
  ...
  console.log('=== STARTING COURSE TRACKER USERID MIGRATION ===');
  ...
  try {
    ...
    for (let i = 0; i < DEST_USER_IDS.length; i++) {
      ...
      // ── OPERATION 1: Fetch all user_course_certificate entries for destUserId ──
      const destEntries = await client.query(
        `SELECT "usercertificateId", "courseId", "status"
         FROM public."user_course_certificate"
         WHERE "userId" = $1`,
        [destUserId]
      );

      if (destEntries.rows.length === 0) {
        console.warn(`  ⚠️  No entries found for destUserId. Skipping this pair.`);
        totalNoEntries++;
        pairsProcessed++;
        console.log('═'.repeat(80));
        continue;
      ...
      console.log(`  Found ${destEntries.rows.length} entry(ies) for destUserId.`);
      ...
      for (const entry of destEntries.rows) {
        const certId = entry.usercertificateId;
        const courseId = entry.courseId;
        const destStatus = entry.status;

        console.log(`\n  ┌─ usercertificateId : ${certId}`);
        console.log(`  │  courseId           : ${courseId}`);

        // ── OPERATION 2: Check if sourceUserId already has this courseId ──
        const sourceCheck = await client.query(
          `SELECT "usercertificateId", "status"
           FROM public."user_course_certificate"
           WHERE "userId" = $1 AND "courseId" = $2`,
          [sourceUserId, courseId]
        );

        if (sourceCheck.rows.length > 0) {
          for (const sourcerow of sourceCheck.rows) {
            const sourceStatus = sourcerow.status;
            const sourceCertId = sourcerow.usercertificateId;
            ...
            if (destPriority > sourcePriority) {
              // destUser has higher status → upgrade sourceUser's status only
              const upgradeResult = await client.query(
                `UPDATE public."user_course_certificate"
               SET "status" = $1
               WHERE "usercertificateId" = $2`,
                [destStatus, sourceCertId]
              );
              if (upgradeResult.rowCount > 0) {
                console.log(`  └─ ⬆️  STATUS UPGRADED: ${sourceStatus} → ${destStatus} (usercertificateId ${sourceCertId})`);
                pairStatusUpgraded++;
                totalStatusUpgraded++;
              } else {
                console.warn(`  └─ ⚠️  STATUS UPGRADE had no effect for usercertificateId: ${sourceCertId}`);
              }
            } else {
              // sourceUser already has same or higher status → skip
        ...
        // sourceUserId has NO record for this courseId → UPDATE userId
        console.log(`  │  ✅ sourceUser has no record for this courseId → updating userId.`);
        ...
        if (updateResult.rowCount > 0) {
          console.log(`  └─ 🔄 UPDATED: usercertificateId ${certId} → userId is now ${sourceUserId}`);
          pairUpdatedUserId++;
          totalUpdatedUserId++;
        } else {
          console.warn(`  └─ ⚠️  UPDATE had no effect for usercertificateId: ${certId}`);
        }
      }
...
if (require.main === module) {
  console.log('Running course-tracker-userid-migration.js directly');
  migrateCourseTrackerUserIds().catch((err) => {
    console.error('[MIGRATION] Migration failed with unhandled error:', err);
    process.exit(1);
  });
}

module.exports = { migrateCourseTrackerUserIds };
