...
console.log('=== Loading coursetracker-userid-migration.js ===');
...
async function migrateCourseTrackerUserIds() {
  ...
  console.log('=== STARTING CourseTracker USERID MIGRATION ===');
  ...
  try {
    ...
    for (let i = 0; i < DEST_USER_IDS.length; i++) {
      ...
      // ── OPERATION 1: Fetch all CourseTracker entries for destUserId ──────────
      const destEntries = await client.query(
        `SELECT "CourseTrackerID", "CourseID", "CourseTrackingStatus"
         FROM public."CourseTracker"
         WHERE "UserID" = $1`,
        [destUserId]
      );

      if (destEntries.rows.length === 0) {
        console.warn(`  ⚠️  No CourseTracker entries found for destUserId. Skipping this pair.`);
        totalNoEntries++;
        pairsProcessed++;
        console.log('═'.repeat(80));
        continue;
      ...
      console.log(`  Found ${destEntries.rows.length} CourseTracker entry(ies) for destUserId.`);
      ...
      for (const entry of destEntries.rows) {
        console.log("HI")
        const trackerId = entry.CourseTrackerID;
        const courseId = entry.CourseID;
        const destStatus = entry.CourseTrackingStatus;

        console.log(`\n  ┌─ CourseTrackerID       : ${trackerId}`);
        console.log(`  │  CourseID              : ${courseId}`);

        // ── OPERATION 2: Check if sourceUserId already has this CourseID ────────
        const sourceCheck = await client.query(
          `SELECT "CourseTrackerID", "CourseTrackingStatus"
           FROM public."CourseTracker"
        ...
        if (sourceCheck.rows.length > 0) {
          ...
          for (const sourceRow of sourceCheck.rows) {
            const sourceStatus = sourceRow.CourseTrackingStatus;
            const sourceTrackerId = sourceRow.CourseTrackerID;
            ...
            if (destPriority > sourcePriority) {
              // destUser has higher status → upgrade sourceUser's CourseTrackingStatus only
              const upgradeResult = await client.query(
                `UPDATE public."CourseTracker"
                 SET "CourseTrackingStatus" = $1
                 WHERE "CourseTrackerID" = $2`,
                [destStatus, sourceTrackerId]
              );
              if (upgradeResult.rowCount > 0) {
                console.log(`  └─ ⬆️  STATUS UPGRADED: ${sourceStatus} → ${destStatus} (CourseTrackerID ${sourceTrackerId})`);
                pairStatusUpgraded++;
                totalStatusUpgraded++;
              } else {
                console.warn(`  └─ ⚠️  STATUS UPGRADE had no effect for CourseTrackerID: ${sourceTrackerId}`);
              }
            } else {
        ...
        const updateResult = await client.query(
          `UPDATE public."CourseTracker"
           SET "UserID" = $1
           WHERE "CourseTrackerID" = $2`,
          [sourceUserId, trackerId]
        );
        if (updateResult.rowCount > 0) {
          console.log(`  └─ 🔄 UPDATED: CourseTrackerID ${trackerId} → UserID is now ${sourceUserId}`);
          pairUpdatedUserId++;
          totalUpdatedUserId++;
        } else {
          console.warn(`  └─ ⚠️  UPDATE had no effect for CourseTrackerID: ${trackerId}`);
        }
      }
    ...
    // ── Final Summary ────────────────────────────────────────────────────────
    console.log('\n╔══════════════════════════════════════════════════╗');
    console.log('║       FINAL CourseTracker MIGRATION SUMMARY      ║');
    console.log('╠══════════════════════════════════════════════════╣');
...
if (require.main === module) {
  console.log('Running coursetracker-userid-migration.js directly');
