...
console.log('=== Loading contenttracker-userid-migration.js ===');

// Status priority: higher number = higher/better status
...
//   OPERATION 1 : Fetch all ContentTracker rows WHERE "UserID" = destUserId
//   OPERATION 2 : For each entry's "ContentID":
//     a) sourceUser has NO record for that ContentID
//          → UPDATE: set "UserID" = sourceUserId on that row (using composite PK)
//     b) sourceUser ALREADY has a record for that ContentID
//          → Check ContentTrackingStatus priority:
//              destUser priority > sourceUser priority
//                → UPDATE sourceUser's ContentTrackingStatus to destUser's status
//              else
//                → SKIP (do nothing)
const DEST_USER_IDS = [
...
async function migrateContentTrackerUserIds() {
  ...
  console.log('=== STARTING ContentTracker USERID MIGRATION ===');
  ...
  let totalUpdatedUserId = 0;
  let totalStatusUpgraded = 0;
  let totalSkipped = 0;
  ...
  try {
    ...
    for (let i = 0; i < DEST_USER_IDS.length; i++) {
      ...
      // ── OPERATION 1: Fetch all ContentTracker entries for destUserId ──────────
      const destEntries = await client.query(
        `SELECT "ContentTrackerID", "ContentID", "ContentTrackingStatus"
      ...
      if (destEntries.rows.length === 0) {
        console.warn(`  ⚠️  No ContentTracker entries found for destUserId. Skipping this pair.`);
        totalNoEntries++;
        pairsProcessed++;
        console.log('═'.repeat(80));
        continue;
      ...
      console.log(`  Found ${destEntries.rows.length} ContentTracker entry(ies) for destUserId.`);

      let pairUpdatedUserId = 0;
      let pairStatusUpgraded = 0;
      let pairSkipped = 0;

      for (const entry of destEntries.rows) {
        const contentTrackerIdDest = entry.ContentTrackerID;
        const contentId = entry.ContentID;
        const destStatus = entry.ContentTrackingStatus;

        console.log(`\n  ┌─ ContentTrackerID       : ${contentTrackerIdDest}`);
        console.log(`  │  ContentID              : ${contentId}`);
        console.log(`  │  destUser status        : ${destStatus}`);

        // ── OPERATION 2: Check if sourceUser already has this ContentID ───────────
        ...
        if (sourceCheck.rows.length > 0) {
          for (const sourceRow of sourceCheck.rows) {
            const contentTrackerIdSrc = sourceRow.ContentTrackerID;
            const sourceStatus = sourceRow.ContentTrackingStatus;
            const destPriority = getStatusPriority(destStatus);
            const sourcePriority = getStatusPriority(sourceStatus);

            if (destPriority > sourcePriority) {
              // destUser has higher status → upgrade sourceUser's ContentTrackingStatus only
              const upgradeResult = await client.query(
                `UPDATE public."ContentTracker"
                 SET "ContentTrackingStatus" = $1
                 WHERE "ContentTrackerID" = $2`,
                [destStatus, contentTrackerIdSrc]
              );
              if (upgradeResult.rowCount > 0) {
                console.log(`  └─ ⬆️  STATUS UPGRADED: ${sourceStatus} → ${destStatus} (ContentTrackerID: ${contentTrackerIdSrc})`);
                pairStatusUpgraded++;
                totalStatusUpgraded++;
              } else {
                console.warn(`  └─ ⚠️  STATUS UPGRADE had no effect for ContentTrackerID: ${contentTrackerIdSrc}`);
              }
            } else {
              console.log(`  │  ⏭️  SKIP: sourceUser status (${sourceStatus}) is same or higher than destUser (${destStatus}).`);
        ...
        // sourceUser has NO record for this ContentID → UPDATE UserID
        console.log(`  │  ✅ sourceUser has no record for this ContentID → updating UserID.`);
        const updateResult = await client.query(
          `UPDATE public."ContentTracker"
           SET "UserID" = $1
           WHERE "ContentTrackerID" = $2`,
          [sourceUserId, contentTrackerIdDest]
        );
        if (updateResult.rowCount > 0) {
          console.log(`  └─ 🔄 UPDATED: ContentTrackerID ${contentTrackerIdDest} → UserID is now ${sourceUserId}`);
          pairUpdatedUserId++;
          totalUpdatedUserId++;
        } else {
          console.warn(`  └─ ⚠️  UPDATE had no effect for ContentTrackerID: ${contentTrackerIdDest}`);
        }
      }

      pairsProcessed++;
      console.log(`\n  [PAIR ${i + 1} RESULT] UserID updated: ${pairUpdatedUserId} | Status upgraded: ${pairStatusUpgraded} | Skipped: ${pairSkipped}`);
      console.log('═'.repeat(80));
    }

    // ── Final Summary ────────────────────────────────────────────────────────
    console.log('\n╔══════════════════════════════════════════════════╗');
    console.log('║     FINAL ContentTracker MIGRATION SUMMARY       ║');
    console.log('╠══════════════════════════════════════════════════╣');
    ...
    console.log(`║  Rows updated (UserID changed)   : ${String(totalUpdatedUserId).padEnd(10)} ║`);
    console.log(`║  Rows status upgraded            : ${String(totalStatusUpgraded).padEnd(10)} ║`);
    console.log(`║  Rows skipped (same/higher)      : ${String(totalSkipped).padEnd(10)} ║`);
    console.log('╚══════════════════════════════════════════════════╝');
...
if (require.main === module) {
  console.log('Running contenttracker-userid-migration.js directly');
  migrateContentTrackerUserIds().catch((err) => {
    console.error('[MIGRATION] Migration failed with unhandled error:', err);
    process.exit(1);
  });
}

module.exports = { migrateContentTrackerUserIds };
